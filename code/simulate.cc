...
// ================= 终极版仪表盘：油耗+碰撞+模式+小地图+档位 =================
  if (this->m && this->d) {
    auto [win_w, win_h] = this->platform_ui->GetFramebufferSize();
    mjrRect viewport = {0, 0, static_cast<int>(win_w), static_cast<int>(win_h)};
    ...
    // [逻辑A] 驾驶模式 (Mode)
    const char* mode_str = "ECO";
    double consumption_rate = 0.005;
    double ax = this->d->qacc[0];
    double ay = this->d->qacc[1];
    double acc_val = std::sqrt(ax*ax + ay*ay);

    if (speed_kmh > 60.0 || acc_val > 2.0) {
        mode_str = "SPORT";
        consumption_rate = 0.02;
    }

    // [逻辑B] 档位计算 (Gear) - 模拟自动挡逻辑
    char gear_char = 'N';
    if (speed_kmh > 1.0) {
        if (speed_kmh < 15) gear_char = '1';
        else if (speed_kmh < 30) gear_char = '2';
        else if (speed_kmh < 50) gear_char = '3';
        else if (speed_kmh < 80) gear_char = '4';
        else gear_char = '5';
    ...
    // [逻辑C] 油耗计算 (Fuel)
    static double fuel_percent = 100.0;
    if (speed_kmh > 0.1) {
        fuel_percent -= consumption_rate * (speed_kmh / 100.0);
    }
    if (fuel_percent < 0.0) fuel_percent = 100.0; // 自动加油

    // [逻辑D] 碰撞检测 (Collision)
    bool is_crashed = false;
    int chassis_id = mj_name2id(this->m, mjOBJ_GEOM, "chassis");
    for (int i = 0; i < this->d->ncon; i++) {
        const mjContact& con = this->d->contact[i];
        if (chassis_id != -1) {
            if (con.geom1 == chassis_id || con.geom2 == chassis_id) {
                is_crashed = true; break;
            }
        } else {
            if (this->d->ncon > 4) is_crashed = true; // 降级方案
        }
    }

    // ---------------------- 2. 渲染绘制流程 ----------------------

    // [绘制A] 小地图 (Mini Map) - 放在右下角
    // 技巧：先画小地图，这样它就是背景，不会遮挡后面的文字
    {
        // 调整位置：放在右下角，稍微抬高一点给油量条留位置
        mjrRect map_rect = {
            static_cast<int>(win_w) - 210, 
            80, 
            200, 
            150
        };
        // 画深色背景框
        mjr_rectangle(map_rect, 0.1f, 0.1f, 0.1f, 0.8f);
        
        // 设置上帝视角摄像机
        mjvCamera map_cam;
        mjv_defaultCamera(&map_cam);
        map_cam.type = mjCAMERA_TRACKING;
        int car_id = mj_name2id(this->m, mjOBJ_BODY, "car");
        map_cam.trackbodyid = (car_id >= 0) ? car_id : 1; 
        map_cam.distance = 15.0;
        map_cam.elevation = -90; // 垂直向下
        map_cam.azimuth = 90;

        // 更新场景并渲染到小方块里
        mjv_updateScene(this->m, this->d, &this->opt, NULL, &map_cam, mjCAT_ALL, &this->scn);
        mjr_render(map_rect, &this->scn, &this->platform_ui->mjr_context());
    }

    // [绘制B] 碰撞全屏警告
    if (is_crashed) {
        mjrRect alert_rect = { static_cast<int>(win_w)/2 - 150, static_cast<int>(win_h)/2 - 50, 300, 100 };
        mjr_rectangle(alert_rect, 0.8f, 0.0f, 0.0f, 0.8f);
        mjr_overlay(mjFONT_BIG, mjGRID_TOPLEFT, alert_rect, "COLLISION DETECTED!", nullptr, &this->platform_ui->mjr_context());
    }

    // [绘制C] 左下角综合仪表盘 (文字)
    double rpm = speed_kmh * (strcmp(mode_str, "SPORT") == 0 ? 60.0 : 45.0);
    if (rpm > 7000) rpm = 7000 + (std::rand() % 200);
    ...
    std::snprintf(dashboard_text, sizeof(dashboard_text), 
        ...
        "RPM   : %-5.0f rpm\n"  // 转速
        "FUEL  : %-5.1f %%",    // 油量数值
        mode_str, gear_char, speed_kmh, rpm, fuel_percent);

    mjr_overlay(mjFONT_BIG, mjGRID_BOTTOMLEFT, viewport, dashboard_text, nullptr, &this->platform_ui->mjr_context());

    // [绘制D] 右下角油量条 (图形)
    mjrRect fuel_bar = {
        static_cast<int>(win_w) - 220, 
        50, // 放在小地图下面
        static_cast<int>(fuel_percent) * 2, 
        20
    };
    // 颜色逻辑：低油量变红
    if (fuel_percent < 20) mjr_rectangle(fuel_bar, 0.8f, 0.0f, 0.0f, 0.8f);
    else mjr_rectangle(fuel_bar, 0.0f, 0.8f, 0.0f, 0.8f);
    
    mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMRIGHT, viewport, "Fuel Level", nullptr, &this->platform_ui->mjr_context());
  }
  // ================= 终极版代码 (结束) =================
